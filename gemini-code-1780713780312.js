...
function nextRound() {
    ...
    const hostRoundEl = document.getElementById('host-current-round');
    if (hostRoundEl) hostRoundEl.innerText = currentRound;
...
function resetFullGame() {
    if (!confirm("هل أنت متأكد من إعادة تصفير الجيم والكلمة بالكامل للجولة 1 وفتح المحاولات مجدداً؟")) return;
    currentRound = 1;
    ...
    const hostRoundEl = document.getElementById('host-current-round');
    if (secretWordEl) secretWordEl.value = "";
    if (hostRoundEl) hostRoundEl.innerText = 1;
