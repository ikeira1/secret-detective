...
function listenToPlayers() {
    database.ref('rooms/' + roomCode + '/players').on('value', (snapshot) => {
        ...
        for (let idKey in players) {
            ...
            // حماية لعدم طباعة كلمة undefined إذا واجه الجوال حالة النوم الخلفي
...
// ⏱️ دالة الترتيب الحي للأسرع بالأجزاء من الثانية وفصل الجولات كلياً
function listenToChallengeAnswers() {
    database.ref('rooms/' + roomCode + '/players').on('value', (snapshot) => {
        ...
        // مصفوفة لتجميع الأجوبة الحالية الموثقة بالوقت التاريخي
        let allAnswers = [];

        for (let idKey in players) {
            const p = players[idKey];
            // التحقق من وجود أرشيف التواريخ والأجوبة للاعب
        ...
        // فرز المجموعات حسب رقم الجولة تنازلياً، وحسب الوقت تصاعدياً (الأسرع أولاً)
        allAnswers.sort((a, b) => {
            if (a.round !== b.round) {
                return b.round - a.round; // الجولة الأحدث تظهر فوق
            }
            return a.timestamp - b.timestamp; // الأسرع بالوقت يظهر فوق داخل نفس الجولة
        });
        ...
        allAnswers.forEach(item => {
            // رسم خط فاصل ملون وجميل إذا تغيرت الجولة بالصندوق
            if (item.round !== lastRenderedRound) {
                lastRenderedRound = item.round;
                rankInRound = 1; // تصفير عداد السرعة مع الجولة الجديدة
            ...
            // تحويل الـ Timestamp الرقمي لساعة دقيقة ثانية ملي ثانية واضحة ومقروءة
            ...
            // وسم خاص وتلوين مميز للأسرع رقم 1 في الجولة
...
function saveSecretWord() {
    ...
    database.ref('rooms/' + roomCode).update({
        ...
        // إرسال رسالة آلية في الشات تنبه اللاعبين ببدء الجيم والتحزير الحقيقي
...
// ➡️ دالة الانتقال للجولة التالية مع الحفاظ على الكلمة السرية ثابتة ومؤمنة
function nextRound() {
    currentRound++;
    
    database.ref('rooms/' + roomCode).update({
        currentRound: currentRound,
        gameStatus: "playing", // تظل حالة اللعب مستمرة وتخمين الكلمة شغال
        winnerWordPlayer: ""
    });

    // تصفير الخانات المؤقتة للتحدي الفرعي فقط وفتح محاولات تخمين إضافية إن أردت
    database.ref('rooms/' + roomCode + '/players').once('value', (snapshot) => {
        const players = snapshot.val();
        if (players) {
            for (let idKey in players) {
                database.ref('rooms/' + roomCode + '/players/' + idKey).update({
                    challengeAnswer: "" // تصفير الجواب النصي المؤقت فقط لتجهيز تحدي جديد
                });
            }
        }
    });
...
// دالة تصفير تحدي الجولة الحالية فقط في حال تراجع المدير بالبث عن تحدي معين
